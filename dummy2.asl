...
+see(b) : haveball(b) & not seegoal(g) <- .print("have ball but not seeing goal"); kicktowardsgoal(g); look.
...
+see(b) : seeball(b) & not closest(b) <- .print("Saw ball, not closest..."); look.
									
+see(_) <- .print("dummy.see nothing (_)"); look.
